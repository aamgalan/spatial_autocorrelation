# import timeit, os, sys, time
import scipy as sp, numpy as np # pandas as pd, itertools as it
from scipy.spatial import Delaunay
from scipy.cluster.hierarchy import dendrogram, linkage
from disjoint_set import DisjointSet
# from collections import defaultdict # from matplotlib import pyplot as plt
# import pysal # from pysal.explore import esda
...
def get_edges_from_linkage_matrix(Z, n_node):
    ...
    for i_cluster, cluster in enumerate(Z):
        ...
        edges.append([int(node) if node<n_node else cluster_to_node[node] for node in cluster[:2]])
...
def get_mst_merge_order(coordinates):
    ...
    for delaunay_edge in delaunay_edges:
        delaunay_edges_weighted.append([
            delaunay_edge[0],
            delaunay_edge[1],
            np.linalg.norm(coordinates[delaunay_edge[0]] - coordinates[delaunay_edge[1]])
        ])
...
def SkienaA(quantities, merge_order, rescaled_scalar=True):
    ...
    for vertex_1, vertex_2 in merge_order:
        ...
        mean_12 = (cluster_info_1['size']*cluster_info_1['mean'] + cluster_info_2['size']*cluster_info_2['mean'])/(cluster_info_1['size'] + cluster_info_2['size'])
        
        mean_change_1 = mean_12 - cluster_info_1['mean']
        mean_change_2 = mean_12 - cluster_info_2['mean']
        
        cluster_variance_change_1 = cluster_info_1['size']*mean_change_1*mean_change_1
        cluster_variance_change_2 = cluster_info_2['size']*mean_change_2*mean_change_2
